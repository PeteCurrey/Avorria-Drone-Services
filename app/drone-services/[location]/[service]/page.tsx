...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const fullUrl = `/drone-services/${location}/${service}`
...
export default async function ServiceLocationPage({ params }: PageProps) {
  ...
  const pageDef = getSeoPageByUrl(fullUrl)
  const content = buildSeoPageContent(fullUrl, location, undefined, service)
  ...
  return (
    <main className="min-h-screen bg-dark pb-32">
      ...
      {/* Related Services */}
      <section className="py-24 px-10 md:px-20 bg-mid border-y border-white/5">
        <div className="max-w-[1200px] mx-auto">
          <h3 className="font-display text-3xl text-white uppercase tracking-widest mb-10">Related Services</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {content.relatedServices.map(svc => (
              <Link 
                key={svc.slug} 
                href={`/services/${svc.slug}`}
                className="p-8 border border-white/5 bg-dark hover:border-accent/30 transition-all group"
              >
                <h4 className="font-display text-xl text-white mb-6 uppercase tracking-widest group-hover:text-accent transition-colors">{svc.name}</h4>
                <div className="font-ui text-[9px] tracking-[0.3em] uppercase text-accent flex items-center gap-2 group-hover:gap-4 transition-all">
      ...
      <section className="mt-20 px-10 md:px-20 text-center max-w-4xl mx-auto">
        ...
        <Link 
          href={`/brief?service=${service}&location=${location}`} 
