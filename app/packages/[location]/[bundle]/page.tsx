...
interface PageProps {
  params: Promise<{ location: string; bundle: string }>
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { location, bundle } = await params
  const fullUrl = `/packages/${location}/${bundle}`
...
export default async function PackageLocationPage({ params }: PageProps) {
  const { location, bundle } = await params
  const fullUrl = `/packages/${location}/${bundle}`
  
  const pageDef = getSeoPageByUrl(fullUrl)
  const content = buildSeoPageContent(fullUrl, location, undefined, undefined, bundle)
  ...
  return (
    <main className="min-h-screen bg-dark pb-32">
      ...
      <section className="mt-20 px-10 md:px-20 text-center max-w-4xl mx-auto">
        ...
        <Link 
          href={`/brief?location=${location}&package=${bundle}`} 
